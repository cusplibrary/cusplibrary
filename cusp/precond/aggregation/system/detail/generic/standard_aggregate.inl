...
void standard_aggregation(thrust::execution_policy<DerivedPolicy> &exec,
                          const MatrixType& C,
                          ArrayType& aggregates,
                          ArrayType& mis,
                          cusp::coo_format)
{
    ...
    ArrayType sorted_aggregates(aggregates);
    ArrayType aggregate_counts(num_aggregates);
    ArrayType reduced_aggregates(num_aggregates);
    ...
    if ( num_singletons > 0 ) {
        ArrayType aggregate_ids(num_aggregates);
