                            ...
                            BinaryFunction2 reduce)
{
    ...
    {
        IndexIterator1 r0 = row_offsets;
        r0 += i;
        IndexType1 row_start = CUSP_DEREFERENCE(r0); // row_offsets[i]
        IndexIterator1 r1 = row_offsets;
        r1 += i + 1;
        IndexType1 row_end   = CUSP_DEREFERENCE(r1); // row_offsets[i + 1]
        ValueIterator3 y0 = y;
        y0 += i;
        ValueType4 sum       = CUSP_DEREFERENCE(y0); // sum = y[i]

        for (IndexType2 jj = row_start; jj < row_end; jj++)
        {
            IndexIterator2 c0 = column_indices;
            c0 += jj;
            IndexType2 j    = CUSP_DEREFERENCE(c0);  // j    = column_indices[jj]
            ValueIterator1 v0 = values;
            v0 += jj;
            ValueType1 A_ij = CUSP_DEREFERENCE(v0);  // A_ij = values[jj]
            ValueIterator2 x0 = x;
            x0 += j;
            ValueType2 x_j  = CUSP_DEREFERENCE(x0);  // x_j  = x[j]
        ...
        ValueIterator4 z0 = z;
        z0 += i;
        CUSP_DEREFERENCE(z0) = sum;                                // z[i] = sum
    }
}
