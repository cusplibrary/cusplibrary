...
public:
    Buffer() : mBuffer(NULL), mAlloc(0), mItems(0) {
        Resize( 4 );
    }
    Buffer( size_t size ) : mBuffer(NULL), mAlloc(0), mItems(0) {
        Resize( size );
    }
    ...
    void EnsureCapacity( size_t capacity ) {
        if ( capacity >= mAlloc ) Resize( capacity * 2 );
    }
    T* Last() {
        return ( &mBuffer[ mItems - 1 ] );
    }
    void Push( const T& item ) {
        EnsureCapacity( mItems + 1 );
        mBuffer[ mItems++ ] = ( item );
    }
    T& Pop() {
    ...
    template< class Mapto > void ForEach( Mapto mapto, size_t limit ) {
        ForEachByRef( mapto, limit );
    }
    template< class Mapto > void ForEach( Mapto mapto ) {
        ForEachByRef( mapto, mItems );
    }
...
public:
    ...
    public:
        ...
        void check( u64Enum e, unsigned long u ) {
            if ( u64fields[e] < u ) u64fields[e] = u;
            if ( e == Calls ) u64fields[TotalCalls] += u;
        }
        void check( f64Enum e, double f ) {
            if ( f64fields[e] < f ) f64fields[e] = f;
        }

        const unsigned long &operator() ( u64Enum e ) const {
            return u64fields[e];
        }
        const double &operator() ( f64Enum e ) const {
            return f64fields[e];
        }
    ...
    *
    ...
    struct ComputeChildTicks
    {
        ...
        ComputeChildTicks( Caller &totals ) : mTotals(totals) {
            maxStats.reset();
        }
...
void detect( int argc, const char *argv[] ) {
    detectByArgs( argc, argv );
}
void detect( const char *commandLine ) {
    detectWinMain( commandLine );
}
void dump() {
    dumpThreads( PrintfDumper() );
}
